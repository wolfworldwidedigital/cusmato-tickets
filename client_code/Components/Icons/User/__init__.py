...
class User(UserTemplate):
  ...
  def set_size(self, value):
    ...
    self.dom_nodes['icon-background'].style.borderRadius = f"{(value or 10)/2}px"
  size = Properties.property_with_callback(set_size)
  
  def set_background(self, value):
    color = Properties.theme_color_to_css(value)
    self.dom_nodes['icon-background'].style.background = color
  background = Properties.property_with_callback(set_background)
